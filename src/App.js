...
const App = () => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        ...
        weatherapi = await axios.get(`https://cors-anywhere.herokuapp.com/https://api.weatherapi.com/v1/current.json?key=61dfb61c8ab64b2aad7101226240703&aqi=no&q=${locationName}`);
      } catch (err) {
        setError(true);
      } finally {
        setLoading(false);
  ...
  const handleSubmit = async (e = 1) => {
    ...
    if (search == "") {
      ...
      const options = {
        method: 'GET',
        url: `https://cors-anywhere.herokuapp.com/https://api.weatherapi.com/v1/current.json?key=61dfb61c8ab64b2aad7101226240703&aqi=no&q=${search}`,
      };
